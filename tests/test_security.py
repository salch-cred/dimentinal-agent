...
def run_audit():
    ...
    if FAIL > 0:
        raise AssertionError(f"Security audit failed: {FAIL} checks failed")


# Wrapper for unittest discovery to run it properly
import unittest

class TestSecurityAudit(unittest.TestCase):
    def test_run_security_audit(self):
        # We suppress stdout to keep the console output neat during unittest discover
        # but let's run it.
        run_audit()
